...
if (games.length === 0) {
  console.log(`${ymd}: no men's college basketball games today; leaving snapshot unchanged`);
  process.exit(0);
}
writeFileSync(
  "data/latest.json",
  JSON.stringify({ sport: "basketball", league: "mens-college-basketball", date: ymd, count: games.length, games }, null, 2) + "\n",
);
console.log(`${ymd}: wrote ${games.length} men's college basketball games -> data/latest.json`);
