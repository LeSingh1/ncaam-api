...
const WEB  = "https://site.web.api.espn.com/apis/common/v3/sports/basketball/mens-college-basketball";
...
async function get(url) {
  const res = await fetch(url, { headers: { "user-agent": "ncaam-api (github.com/LeSingh1/ncaam-api)" } });
  if (!res.ok) throw new Error(`ESPN ${res.status} for ${url}`);
  return res.json();
}

// men's college basketball is basketball/mens-college-basketball on ESPN.
